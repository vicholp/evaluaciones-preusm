...
<div class="container mx-auto grid grid-cols-12 p-3 gap-3 text-black text-opacity-90">
  <div class="col-span-12 flex flex-row items-center gap-3">
    ...
    <a href="{{ route('admin.periods.edit', $period) }}" class="bg-blue-800 rounded p-3 text-white inline-block">
      Edit
    </a>
    <form action="{{ route('admin.periods.destroy', $period) }}" method="POST" hidden id="form-delete">
      @csrf
      @METHOD('DELETE')
    </form>
  ...
  <div class="col-span-12 bg-white rounded shadow p-3 flex flex-col gap-3">
    <div class="flex flex-col gap-4 p-3">
      <div class="grid grid-cols-12">
        ...
        <div class="col-span-8 text-black"> {{ $period->id }} </div>
      </div>
      <div class="grid grid-cols-12">
        <div class="col-span-4 text-black text-opacity-90"> name </div>
        <div class="col-span-8 text-black"> {{ $period->name }} </div>
      </div>
      <div class="grid grid-cols-12">
        <div class="col-span-4 text-black text-opacity-90"> start_date </div>
        <div class="col-span-8 text-black"> {{ $period->start_date }} </div>
      </div>
      <div class="grid grid-cols-12">
        <div class="col-span-4 text-black text-opacity-90"> end_date </div>
        <div class="col-span-8 text-black"> {{ $period->end_date }} </div>
      </div>
    </div>
  </div>
</div>
