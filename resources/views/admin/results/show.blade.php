      ...
      <x-base.card padding="false">
        <x-base.table>
          @foreach ($results->childs as $result)
            <x-base.table.row>
              <div class="col-span-4"> {{ $result->data['rut'] ?? $result->data['email'] }}</div>
              <div class="col-span-2"> {{ $result->result }}</div>
              <div class="col-span-6"> {{ $result->result != 'success' ? collect($result->log)->join(', ') : 'OK' }} </div>
            </x-base.table.row>
          @endforeach
        </x-base.table>
      </x-base.card>
