  ...
  <x-teacher.container>
    ...
    >
      <x-slot:actions>
        ...
        @if ($question->pilot)
          <x-base.action
            :href="route('teacher.results.questions.unmark-as-pilot', $question)"
            :body="__('unmark as pilot')"
          />
        @else
          <x-base.action
            :href="route('teacher.results.questions.mark-as-pilot', $question)"
      ...
      <x-teacher.card.card>
        <x-teacher.card.list :divide="false">
          <x-teacher.card.list-key-value
            :key="Str::ucfirst(__('position'))"
            :value="$question->position"
          />
          <x-teacher.card.separator />
          <x-teacher.card.list-key-value
            :key="Str::ucfirst(__('skill'))"
            :value="$question->skills->first()?->name ?? 'n/a'"
          />
          <x-teacher.card.list-key-value
            :key="Str::ucfirst(__('topic'))"
            :value="$question->topics->first()?->name ?? 'n/a'"
          />
          <x-teacher.card.list-key-value
            :key="Str::ucfirst(__('subtopic'))"
            :value="$question->subtopics->first()?->name ?? 'n/a'"
          />
          <x-teacher.card.list-key-value
            :key="Str::ucfirst(__('item type'))"
            :value="$question->itemTypes->first()?->name ?? 'n/a'"
          />
          <x-teacher.card.list-key-value
            :key="str(__('pilot'))->ucfirst()"
            :value="$question->pilot ? 'Si' : 'No'"
          />
          <x-teacher.card.separator />
          <x-teacher.card.list-key-value
            :key="Str::ucfirst(__('answers'))"
            :value="$question->stats()->getAnswerCount()"
          />
          <x-teacher.card.list-key-value
            ...
            <x-teacher.card.table-row>
              ...
              <div class="col-span-1 text-center">
                @if ($alternative->correct)
                  <div class="font-medium text-green-500 text-opacity-100">
                    correcta
                  </div>
                @endif
              </div>
